...
export function ViewDomiciliationDialog({
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const res = await fetch(`/api/user/basic-user/${userId}`, {
        method: "PATCH",
        body: JSON.stringify({
          virtualOfficeName: data.companyName,
          virtualOfficeLegalForm: data.companyLegalForm,
          virtualOfficeSiret: data.siret,
        }),
      });
  ...
  const handleValidateDocument = async (document: Documents) => {
    ...
    try {
      const response = await fetch(
        `/api/upload-file-user/${document.file_type}/${document.id}`,
        {
          method: "PATCH",
          headers: {
  ...
  useEffect(() => {
    if (domiciliation?.contractFiles && domiciliation.status !== 'confirmed') {
      const VirtualOfficeToActivate = domiciliation.contractFiles.filter(
        (v: any) => v.isContractSignedByAdmin === true && v.isContractSignedByUser === true
      );
  ...
  const handleCreateProcedure = async (
    ...
    try {
      ...
      // Lien de signature utilisateur fourni par Yousign
  ...
  const handleSignaturesReady = useCallback(
    ...
    [document, handleCreateProcedure]
  );
  ...
  const handleSign = async (id: string) => {
    try {
      ...
      // Création de la procédure avec les deux signataires
  ...
  const handleSave = async (contractId: string) => {
    try {
      ...
      if (!contractRes.ok || !contractData.success) {
        throw new Error(contractData.error || "Impossible de récupérer le contrat");
      }
  
      const signatureRequestId = String(contractData.data.procedureId);

      console.log(typeof signatureRequestId)
      ...
      if (!saveRes.ok || !saveData.success) {
        throw new Error(saveData.error || "Erreur lors de la sauvegarde du document signé");
      }
        ...
        <div className="w-full h-full mt-8 px-6 py-3 bg-white rounded-xl">
          ...
          <div>
            <div className="space-y-2 py-2">
              ...
              {domiciliation.userFiles.stats.validated === 4 && (
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {isSigned && (
                          <Button
                            ...
                            onClick={() => handleSave(String(document.contractFileId))}
                          >
