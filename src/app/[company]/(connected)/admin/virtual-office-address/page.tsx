...
import VirtualOfficeLists from "./VirtualOfficeLists";
...
export default function DomiciliationPage() {
  ...
  const adminCompanySlud = session?.user.companySlug;

  const fetchVirtualOffices = async () => {
    try {
      const response = await fetch("/api/virtual-office");
      const result = await response.json();
      if (result.success) {
        const virtualOffices = result.data;

        const officeLists = virtualOffices.filter(
          (i: any) => i.status !== "canceled"
        );

        setCompaniesData(officeLists);
        const active = virtualOffices.filter(
          (i: any) => i.status === "confirmed"
        ).length;

        const pending = virtualOffices.reduce(
          (acc: number, i: any) => acc + (i.userFiles?.stats?.pending || 0),
          0
        );
        ...
        const revenue = virtualOffices
          ...
          .reduce(
            (acc: number, i: any) => acc + (parseFloat(i.amount) || 0),
            0
          );
        ...
        const actions = virtualOffices.reduce(
          (acc: number, i: any) =>
            ...
            (i.contractFiles?.filter(
              (file: any) =>
                !file.isContractSignedByAdmin && file.isContractSignedByUser
            ).length || 0),
          0
        );
      ...
      console.error(
        "Erreur lors de la récupération des addresses virtuels :",
        error
      );
    } finally {
  ...
  return (
    <div className="py-8 px-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between sm:flex-row sm:justify-between gap-2 mb-6">
        ...
        <div className="flex gap-3">
          {/* <Button
            ...
            onClick={() =>
              router.push(
                `/${adminCompanySlud}/admin/virtual-office-address/new`
              )
            }
      ...
      {loadingOffersList ? (
        ...
        // TODO: nouveau domiciliation : non obli file + client side ( upload fichier non obli 4)
      )}
    </div>
  );
}
